...
int
read_sign_items(char *fn, struct sign_item_list *head) {
	FILE *f;
	char buf[128], *s;
	...
	for (;;) {
		...
		if (*s == '\0') {
			fprintf(stderr, "%s:%d short line or parse error after reading frame time!\n", fn, line);
			continue;
		}

		dtime = strtoul(s, &s, 0);
		if (*s == '\0') {
			fprintf(stderr, "%s:%d short line or parse error after reading delay time!\n", fn, line);
			continue;
		}
...
static uint32_t
image_getrgb(SDL_Surface *image, unsigned int x, unsigned int y) {
...
static void
image_to_pixels(uint32_t *pixels, SDL_Surface *image, unsigned int width, unsigned int height, unsigned int xoffs)
{
...
main(int argc, char **argv) {
	...
	while ((i = getopt(argc, argv, "hw:h:")) != -1) {
		...
		case 'W': width=atoi(argv[optind]); break;
		case 'H': height=atoi(argv[optind]); break;
		}
	}

	if (optind >= argc) {
		fprintf(stderr, "You have to specify the textfile listing the png images to show on the sign!\n");
		usage(argv[0]);
		exit(1);
	}
	...
	fprintf(stderr, "Initializing %u x %u pixels (%u in total).\n",
			width, height, ledstring.channel[0].count);
	...
	while (keep_running) {
		...
		image_to_pixels(ledstring.channel[0].leds, item->image, width, height, offs_x);
		...
		offs_x++;
		if (((unsigned int)item->image->w) <= width || offs_x > (((unsigned int)item->image->w)-width)) {
//			fprintf(stderr, "Next item, sleeping for %lu us.\n", item->delay_time_us);
			usleep(item->delay_time_us);
			offs_x = 0;
			item = SLIST_NEXT(item, sign_items);
			if (!item) {
				item = SLIST_FIRST(&sign_item_list);
//				fprintf(stderr, "Wraparound!\n");
			}
		}
	}
